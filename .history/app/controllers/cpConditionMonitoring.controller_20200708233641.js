const CpConditionMonitoringm = require('../models/cpConditionMonitoring.model.js');
...
exports.findAll = (req, res) => {
    CpConditionMonitoringm.find({dataStatus  : 1 })
    .then(CpConditionMonitoringm => {
        res.send(CpConditionMonitoringm);
        console.log(CpConditionMonitoringm);
    }).catch(err => {
        res.status(500).send({
...
var id= req.params.primeNumber
    CpConditionMonitoringm.find({primeNumber:id,dataStatus : 1})
    .then(CpConditionMonitoringm => {
        if(!CpConditionMonitoringm) {
        ...
        if(err.kind === 'ObjectId') {
            return res.status(404).send({
                message: "CpConditionMonitoringm not found with id " + req.params.customerDemographicId
            });                
        }
...
// Update a CustomerDemographic identified by the CustomerDemographicId in the request
 exports.update = (req, res) => {
    ...
    CpConditionMonitoringm.updateOne({ "primeNumber": req.body.primeNumber }, 
    { $set : req.body},)
    .then(CpConditionMonitoringm => {
        if(!CpConditionMonitoringm) {
            return res.status(404).send({
...
//<-------------------------------------Audit Trail---------------------------------------------->\\
    ...
    console.log('before find one and delete query ',req.body.primeNumber);
     ...
     CpConditionMonitoringm.findOneAndDelete({primeNumber: req.body.primeNumber})
     .then(cpConditionMonitoring => {
         if(!cpConditionMonitoring) {
             return res.status(404).send({
